...
//
// Usage:
//   node keepalive-render.cjs
//
// PM2 cron (runs every 10 minutes):
//   pm2 start keepalive-render.cjs --cron-restart "*/10 * * * *" --no-autorestart
...
async function main() {
  ...
  const results = await Promise.allSettled(ENDPOINTS.map((endpoint) => ping(endpoint)));

  const successful = results.filter((r) => r.status === 'fulfilled' && r.value.success).length;
  const failed = results.length - successful;

  console.log(`[${new Date().toISOString()}] Completed: ${successful} succeeded, ${failed} failed\n`);
